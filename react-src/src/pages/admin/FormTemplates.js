...
function FormTemplates() {
  ...
  const loadFormTemplates = async () => {
    ...
    try {
      ...
      const res = await apiRequest(`/api/admin/form-templates?${params}`);
      if (res.ok) {
        const data = await res.json();
    ...
    <div>
      <div className="bg-white rounded-xl shadow-sm border border-blue-100 p-6 mb-6">
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-2xl font-bold text-blue-700">Шаблоны форм</h1>
          <button
              ...
              <tbody className="divide-y divide-blue-100">
                {formTemplates.map((template) => (
                  <tr key={template.id} className="hover:bg-blue-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-blue-700">
                      {template.visa_type?.name || "—"}
                      {template.visa_type?.country && (
            ...
            {pagination.last_page > 1 && (
              <div className="px-6 py-4 border-t border-blue-100 flex items-center justify-between">
                <div className="text-sm text-blue-400">
                  Показано {formTemplates.length} из {pagination.total}
                </div>
